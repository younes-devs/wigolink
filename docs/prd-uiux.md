...
| # | Exigence | Critère d'acceptation |
|---|---|---|
| U14 | **i18n socle** : mécanisme t() + bascule FR/AR (RTL) — l'arabe d'abord, le public cible étant la diaspora marocaine ; NL ensuite (Belgique) | **Livré (complet côté app)** : mécanisme t() sans dépendance, dictionnaires FR/AR en fichiers séparés (~460 clés, parité vérifiée au script), sélecteur persisté, dir=rtl avant le rendu, et TOUS les écrans utilisateurs traduits : connexion/inscription, feed + trajets + filtres, création d'annonce (3 étapes + douane), mes envois, détail d'annonce, transactions (liste + détail complet : timeline, QR, scellage, chat, litiges, notation), profil + RGPD, KYC, formation voyageur, onboarding, 404, notifications, SideRail. Dates localisées ar-MA. Propriétés logiques CSS (timeline, stepper) pour un RTL correct. **Restent FR par choix documenté** : back-office admin (équipe interne), outils de démo, PDF/partage douane (documents pour contrôles, pas de glyphes arabes dans jsPDF),  **NL livré aussi** : dictionnaire néerlandais complet (462 clés, parité vérifiée), sélecteur à 3 langues, dates nl-BE via un helper dateLocale() centralisé (les formats Intl figés au niveau module sont devenus des fonctions pour suivre la langue). **Erreurs API traduites aussi** : middleware serveur qui traduit body.error à la sortie selon Accept-Language (table des 81 messages + 4 motifs dynamiques, couverture vérifiée au script, repli FR pour toute langue inconnue). **Catégories localisées aussi** : /api/rules sert les libellés whitelist/blacklist (et raisons) en fr/ar/nl selon Accept-Language ; seules les catégories promues dynamiquement restent dans leur langue de création. |
