...
export default function PayoutSetup() {
  ...
  if (mode === 'manual' && payout?.ready && !editingAccount) {
    ...
    content = <form className="payout-setup-content payout-manual-form" onSubmit={saveManualAccount}>
      ...
      <label className="field"><span>{t('payments.payout.country')}</span><select value={country} onChange={(event) => setCountry(event.target.value)}>{COUNTRIES.filter(([code]) => ['MA', 'BE', 'FR'].includes(code)).map(([code, name]) => <option key={code} value={code}>{countryName(code, name)}</option>)}</select></label>
    ...
    content = <section className="payout-setup-content">
      ...
      <label className="field"><span>{t('payments.payout.country')}</span><select value={country} onChange={(event) => setCountry(event.target.value)} disabled={payout?.configured}>{COUNTRIES.map(([code, name]) => <option key={code} value={code}>{countryName(code, name)}</option>)}</select></label>
