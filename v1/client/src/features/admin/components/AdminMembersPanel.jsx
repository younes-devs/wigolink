...
function MemberCaseFile({ userId, onBack }) {
  ...
  return <div className="admin-case-file">
    ...
    <section className="card"><h2><Icon name="repeat" size={18} />{t('admin.member.activity')}</h2><div className="kyc-recap mt"><div><span className="muted">{t('admin.member.trips')}</span><b>{data.recordTotals?.trips ?? data.trips.length}</b></div><div><span className="muted">{t('admin.member.listings')}</span><b>{data.recordTotals?.listings ?? data.listings.length}</b></div><div><span className="muted">{t('admin.member.operations')}</span><b>{data.recordTotals?.transactions ?? data.transactions.length}</b></div><div><span className="muted">{t('admin.member.disputes')}</span><b>{data.recordTotals?.disputes ?? data.disputes.length}</b></div></div><div className="list-stack mt">{data.transactions.map((transaction) => <div className="list-row" key={transaction.id}><div className="grow"><b>{transaction.id}</b><div className="muted">{adminStatus(transaction.status)} · {formatAdminDate(transaction.createdAt)}</div></div><span className="pill pill-gray">{adminStatus(transaction.escrow?.state || 'pending')}</span></div>)}</div></section>
