...
try {
  ...
  const result = await client.query('select state from public.wigolink_app_state where id = 1 for update');
  if (!result.rows[0]?.state) throw new Error('Etat Wigolink introuvable dans Supabase.');
