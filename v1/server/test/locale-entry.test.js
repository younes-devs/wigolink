...
test('locale entry redirige selon Accept-Language sans cache partage', () => {
  ...
  assert.equal(response.headers.Location, '/es/trajets');
...
test('locale entry prefere le cookie et retombe sur le francais', () => {
  assert.equal(requestRoot({ cookie: 'wigolink_lang=ar', 'accept-language': 'en' }).headers.Location, '/ar/trajets');
  assert.equal(requestRoot({ 'accept-language': 'de-DE' }).headers.Location, '/fr/trajets');
  assert.equal(requestRoot().headers.Location, '/fr/trajets');
});
