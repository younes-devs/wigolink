...
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'users', '[]'::jsonb)) item
where item ? 'id'
on conflict (id) do update set data = excluded.data, updated_at = now();

insert into public.wigolink_trips (id, data, created_at, updated_at)
select item->>'id', item, to_timestamp(coalesce((item->>'createdAt')::double precision, extract(epoch from now()))), now()
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'trips', '[]'::jsonb)) item
where item ? 'id'
on conflict (id) do update set data = excluded.data, updated_at = now();

insert into public.wigolink_listings (id, data, created_at, updated_at)
select item->>'id', item, to_timestamp(coalesce((item->>'createdAt')::double precision, extract(epoch from now()))), now()
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'listings', '[]'::jsonb)) item
where item ? 'id'
on conflict (id) do update set data = excluded.data, updated_at = now();

insert into public.wigolink_transactions (id, data, created_at, updated_at)
select item->>'id', item, to_timestamp(coalesce((item->>'createdAt')::double precision, extract(epoch from now()))), now()
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'transactions', '[]'::jsonb)) item
where item ? 'id'
on conflict (id) do update set data = excluded.data, updated_at = now();

insert into public.wigolink_matching_offers (id, data, created_at, updated_at)
select item->>'id', item, to_timestamp(coalesce((item->>'createdAt')::double precision, extract(epoch from now()))), now()
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'matchingOffers', '[]'::jsonb)) item
where item ? 'id'
on conflict (id) do update set data = excluded.data, updated_at = now();

insert into public.wigolink_saved_trips (id, data, created_at, updated_at)
select item->>'id', item, to_timestamp(coalesce((item->>'createdAt')::double precision, extract(epoch from now()))), now()
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'savedTrips', '[]'::jsonb)) item
where item ? 'id'
on conflict (id) do update set data = excluded.data, updated_at = now();

insert into public.wigolink_conversations (id, data, created_at, updated_at)
select item->>'id', item, to_timestamp(coalesce((item->>'createdAt')::double precision, extract(epoch from now()))), now()
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'conversations', '[]'::jsonb)) item
where item ? 'id'
on conflict (id) do update set data = excluded.data, updated_at = now();

insert into public.wigolink_disputes (id, data, created_at, updated_at)
select item->>'id', item, to_timestamp(coalesce((item->>'createdAt')::double precision, extract(epoch from now()))), now()
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'disputes', '[]'::jsonb)) item
where item ? 'id'
on conflict (id) do update set data = excluded.data, updated_at = now();

insert into public.wigolink_review_queue (id, data, created_at, updated_at)
select item->>'id', item, to_timestamp(coalesce((item->>'createdAt')::double precision, extract(epoch from now()))), now()
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'reviewQueue', '[]'::jsonb)) item
where item ? 'id'
on conflict (id) do update set data = excluded.data, updated_at = now();
...
with ranked_appeals as (
  ...
  from public.wigolink_app_state,
...
insert into public.wigolink_kyc_submissions (id, data, created_at, updated_at)
select item->>'id', item, to_timestamp(coalesce((item->>'createdAt')::double precision, extract(epoch from now()))), now()
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'kycSubmissions', '[]'::jsonb)) item
where item ? 'id'
on conflict (id) do update set data = excluded.data, updated_at = now();

insert into public.wigolink_kyc_decisions (id, data, created_at, updated_at)
select item->>'id', item, to_timestamp(coalesce((item->>'createdAt')::double precision, extract(epoch from now()))), now()
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'kycDecisions', '[]'::jsonb)) item
where item ? 'id'
on conflict (id) do update set data = excluded.data, updated_at = now();

insert into public.wigolink_custom_whitelist (id, data, created_at, updated_at)
select coalesce(item->>'id', 'custom-' || md5(item::text)), item, now(), now()
from public.wigolink_app_state, jsonb_array_elements(coalesce(state->'customWhitelist', '[]'::jsonb)) item
on conflict (id) do update set data = excluded.data, updated_at = now();
...
from public.wigolink_app_state, jsonb_each(coalesce(state->'otps', '{}'::jsonb))
...
from public.wigolink_app_state, jsonb_each(coalesce(state->'resets', '{}'::jsonb))
on conflict (kind, id) do update set data = excluded.data, expires_at = excluded.expires_at, updated_at = now();
...
from public.wigolink_app_state, jsonb_each(coalesce(state->'pendingVerifications', '{}'::jsonb))
on conflict (kind, id) do update set data = excluded.data, expires_at = excluded.expires_at, updated_at = now();
